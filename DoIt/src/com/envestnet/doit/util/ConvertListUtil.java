...
public class ConvertListUtil {
	...
	public static List<TaskBean> convert(ResultSet rs) {
		...
		/*
		 * Prepare a list of Tasks from the given Resultset obtained from the
		 * select statement on the TASK database table
		 */
		...
		try {
			while (rs.next()) {
				...
				t.setTasktime(LocalTime.of(Integer.parseInt(time[0]),
						Integer.parseInt(time[1])));
				t.setTaskdate(LocalDate.of(Integer.parseInt(date[0]),
